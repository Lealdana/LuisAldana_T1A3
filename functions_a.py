...
import time
print(f'{fg(1)}{bg(15)}Welcome to the General knowledge Test Game!!{attr("reset")}\n')
...
def user_confirmation():
    ...
    while True:
        play = input("Hey " + user_name + "! are you ready to start playing? \n \nType Yes or No?:  \n").lower()
        if play == "yes":
            print(" ------------------- \n I knew it! \n  -------------------\n")
            break
        elif play == "no":
...
def loading_game():
    ...
    for i in range(wait_time):
        print(f"{fg(10)}{bg(8)} Loading game.{'.'*i} \n -------------------  {attr('reset')}")
        time.sleep(1)
